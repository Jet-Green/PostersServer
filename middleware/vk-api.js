...
module.exports = process.env.NODE_ENV == 'production' ? {
    async postInGroup(link, eventType) {
        let response = await axios.post(`https://api.vk.com/method/wall.post?access_token=${process.env.VK_ACCESS_TOKEN}&v=5.131&attachments=${link}&message=${eventType.join('|')} ${message}&owner_id=-222755810&from_group=1`)
        return response
    }
} : { async postInGroup(link) { } }
