...
// `https://oauth.vk.com/authorize?client_id=-51783056&scope=manage&redirect_uri=http://localhost:3031&response_type=code`
...
let toExport = process.env.NODE_ENV == 'production' ? {
    async postInGroup(message) {
        let response = await axios.post(`https://api.vk.com/method/wall.post?access_token=${process.env.VK_ACCESS_TOKEN}&v=5.131&message=${message}&owner_id=-222755810&from_group=1`)
        return response
    }
} : { async postInGroup(message) { return } }
